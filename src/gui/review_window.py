...
class ReviewWindow(ttk.Toplevel):
    def __init__(self, parent, data: List[Dict], all_filters: List[str], current_materia_filters: List[str], on_save: Callable):
        ...
        self.data = data 
        # Lista completa (para busca geral)
        self.all_filters = sorted(list(set(all_filters))) if all_filters else []
        # Lista filtrada (apenas da matéria selecionada, para facilitar o início)
    ...
    def _add_tag_widget(self, parent, aula_key, match_data):
        ...
        if term in [x['termo'] for x in self.result_map[aula_key]]:
             # Se já está na lista lógica (result_map) mas ainda não renderizou (ex: load inicial), ok.
             # Mas se usuário tenta adicionar duplicado, não faz nada.
             # Aqui simplificamos: sempre renderiza, mas a lógica de salvar remove duplicatas se houver.
             pass
        ...
        # Adiciona aos dados internos (se não for duplicata lógica)
    ...
    def add_filter_dialog(self, parent_tags, aula_key):
        ...
        # Combobox
        # Inicialmente mostra apenas assuntos da matéria selecionada para não poluir
        ...
        def on_type(event):
            typed = cb.get()
            if typed == '':
                # Se vazio, volta a mostrar os filtros da matéria
                cb['values'] = self.current_materia_filters
            else:
                # Se digitou, busca em TUDO (all_filters)
                typed_lower = typed.lower()
                # Limita a 50 resultados para não travar a interface
                matches = [f for f in self.all_filters if typed_lower in f.lower()][:50]
                cb['values'] = matches
                
                # Opcional: abrir a lista automaticamente ao digitar (pode ser intrusivo, descomente se quiser)
                # if matches: cb.event_generate('<Down>')
